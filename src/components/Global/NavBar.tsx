...
} from "@/../public/images/InfinixChainLogo.png";
...
import { ConnectWallet, Modal, NavSearchBar } from "./";
import ConnectButton from '../../web3/ConnectButton'
import TronNileWallet from '../../app/TronNileWallet'
import ChainSelect from "@/web3/ChainSelect";
...
const NavBar = () => {
  ...
  // console.log("tronCheck Navbar", tronCheck)
  ...
  return (
    <header
      id="header"
      className={`${scrolled ? "header-animation bg-BG" : "absolute"} ${path === "/" && "bg-BG z-20"
        ...
        >
          ...
          <nav className="w-full xl:flex justify-between items-center text-black hidden text-semibold">
              ...
              <ul className="flex justify-center items-center gap-3 lg:gap-5">
                {navMenu?.map((menu, idx) => (
                  <li
                    ...
                    className={`${isMenuActive(menu) ? "text-primary-1" : "text-white"
                  ...
                  >
                    {menu?.isSubMenu ? (
                      <>
                        ...
                        <IconChevronDown
                          className={`sm:w-6 sm:h-6 w-5 h-5 sm:ml-2 ml-1.5 hover:rotate-180 duration-700 ease-in-out text-2xl ${toggle === idx ? "rotate-180" : "rotate-0"
                            }`}
                        />
                      </>
                    ) : (
                    ...
                    {menu?.isSubMenu && (
                      <div className="group w-[500px] z-[300] bg-BG shadow-[0px_1px_12px_0px_#000000b5] text-white p-6 sm:p-8 rounded-3xl opacity-0 invisible group-hover:visible group-hover:opacity-100 absolute top-7 -left-96 my-transition ease-out duration-300">
                        <h3 className="text-primary-0 text-center my-text-20 w-full">
                          Our Products are live on the Testnet.
                        </h3>
                        <div className="flex justify-between flex-wrap gap-4 mt-6 border-2 rounded-xl pt-4">
                          {menu?.subMenu?.map((subItem, index) => (
                            <div
                              key={subItem.id}
                              className={` ${path === subItem.link
                                ? "text-primary-1"
                                : "text-white"
                                }  my-text-18 font-bold font-lexend leading-[130%] hover:text-primary-1 w-[45%] hover:translate-x-1 duration-700 flex flex-col items-center`}
                            >
                              <Link href={subItem.link} className="flex flex-col items-center">
                                <Image
                          ...
                          <ul
                            className={`shadow-[0px_1px_12px_0px_#000000b5]  bg-BG  ${toggle === idx ? "translate-y-0" : "translate-y-8"
            ...
            <div className="flex items-center flex-wrap gap-5 font-inter text-lg font-medium md:leading-[30px] leading-6 relative">
              ...
              {/* <button
                onClick={handleModal}
                className="btn btn-primary xl:p-2 xxl:p-4"
              >
                Connect Wallet
              </button>  */}
            </div>
          ...
          <nav className="w-full flex justify-between items-center text-black xl:hidden">
            <div className="relative">
              ...
              <div
                className={`fixed overflow-y-scroll scrollbar scrollbar-sm top-0 left-0 bg-BG h-screen w-full  shadow-lg z-20 transform transition-transform ease-in-out duration-500 p-8 ${hamburgerToggle
              ...
              >
                ...
                <div className="w-full">
                  <ul className="flex flex-col justify-center items-start gap-5">
                    {navMenu?.map((menu, idx) => (
                      <li
                        className={`  ${(menu.name === "Services" || "Blogs") &&
                          isMenuActive(menu)
                          ? "text-primary-1"
                          : "text-white"
                      ...
                      >
                        ...
                        >
                          <ul className={`pt-2`}>
                            <li
                              className="text-primary-0 my-text-18"
                            >
                              Our Products are live on the Testnet.
                            </li>
